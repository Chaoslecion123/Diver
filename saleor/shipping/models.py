...
class ShippingMethodQueryset(models.QuerySet):
    ...
    def location_based(self):
        return self.filter(type=ShippingMethodType.LOCATION_BASED)
    # END :: SoftButterfly Extensions ------------------------------------------

    def applicable_shipping_methods(self, price, weight, country_code=None, address=None):
        ...
        # in the first place
        if country_code is None and address is not None:
...
class ShippingMethod(models.Model):
    ...
    def get_location_display(self):
        ...
        if self.type == ShippingMethodType.LOCATION_BASED:
            location_display = f"{self.country_area}"

            if self.city:
                location_display = f"{location_display} - {self.city}"

            if self.city_area:
                location_display = f"{location_display} - {self.city_area}"
