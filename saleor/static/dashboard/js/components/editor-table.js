...
import dataTable from '../../images/editor/data_table.svg';
...
function getSelectionStart(doc) {
  var node = doc.getSelection().baseNode
    ? doc.getSelection().baseNode
    : doc.getSelection().anchorNode;
...
Grid.prototype = {
  ...
  markCells: function() {
    [].forEach.call(
      ...
      function(el) {
        var cell = {
          column: parseInt(el.getAttribute('data-column'), 15),
          row: parseInt(el.getAttribute('data-row'), 15)
        };
        var active =
          this._currentCell &&
...
Builder.prototype = {
  ...
  setEditor: function(range, restrictNestedTable) {
    ...
    if (restrictNestedTable) {
      var elements = this._doc.getElementsByClassName(
  ...
  setBuilder: function() {
    ...
    for (var i = 0; i < elements.length; i++) {
      elements[i].style.height =
        COLUMN_WIDTH * this.rows + BORDER_WIDTH * 2 + 'px';
      elements[i].style.width =
        COLUMN_WIDTH * this.columns + BORDER_WIDTH * 2 + 'px';
    }
  },
  ...
  addColumn: function(before, e) {
    ...
    for (var i = 0; i < tbody.childNodes.length; i++) {
      ...
      td.appendChild(this._doc.createElement('br'));
      if (before === true) {
        tbody.childNodes[i].insertBefore(
          td,
          tbody.childNodes[i].childNodes[cell]
        );
      } else if (tbody.childNodes[i].childNodes[cell].nextSibling) {
        tbody.childNodes[i].insertBefore(
          td,
          tbody.childNodes[i].childNodes[cell].nextSibling
        );
      } else {
...
Table.prototype = {
  ...
  insert: function(rows, cols) {
    ...
    if ($(table).find('#medium-editor-table-tbody').length === 0) {
...
MediumEditorTable = MediumEditor.extensions.form.extend({
  name: 'table',
  // aria: 'create table',
  aria: pgettext('Rich text editor option', 'Crear tabla'),
  action: 'table',
  // contentDefault: 'TBL',
  contentDefault: `<img src="${dataTable}">`,
  ...
  show: function() {
    ...
    if (
      ...
      this.builder.setEditor(
        MediumEditor.selection.getSelectedParentElement(range),
        this.restrictNestedTable
      );
    } else {
