...
class Category(MPTTModel, SeoModel):
    ...
    icon_image = VersatileImageField(
        upload_to='category-icons',
        blank=True, )
    icon_image_alt = models.CharField(
        max_length=128,
        blank=True, )
...
class Product(SeoModel, PublishableModel):
    ...
    specs_json = JSONField(blank=True, default=dict)
    ...
    def get_absolute_url(self):
        ...
        return reverse(
            'frontend:storefront-product',
            kwargs={'slug': self.get_slug(), 'id': self.id})
...
class Collection(SeoModel, PublishableModel):
    ...
    def get_absolute_url(self):
        ...
        return reverse(
            'frontend:storefront-collection',
            kwargs={'slug': self.slug, 'id': self.id}
        )
