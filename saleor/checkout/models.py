...
from . import ShippingType, BillingType
...
class Checkout(models.Model):
    ...
    shipping_type = models.CharField(
        max_length=15,
        default=ShippingType.DOOR_SHIPPING,
        choices=ShippingType.CHOICES, )
    billing_type = models.CharField(
        max_length=15,
        default=BillingType.BALLOT,
    ...
    def get_shipping_price(self, taxes):
        ...
        if self.shipping_method and self.is_shipping_required():
            return self.shipping_method.get_total(taxes)
        if getattr(self, 'glovo_order', None):
            return self.glovo_order.get_total(taxes)  # pylint: disable=no-member
        if getattr(self, 'runningbox_order', None):
            return self.runningbox_order.get_total(taxes)  # pylint: disable=no-member
