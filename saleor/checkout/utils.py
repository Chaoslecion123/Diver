...
def set_checkout_cookie(simple_checkout, response):
    ...
    response.set_signed_cookie(
        COOKIE_NAME, simple_checkout.token, max_age=max_age)
...
def remove_unavailable_variants(checkout):
    ...
    for line in checkout:
        try:
            ...
            add_variant_to_checkout(
                checkout, line.variant, quantity, replace=True)
        ...
        def func(request, *args, **kwargs):
            ...
            if request.user.is_authenticated:
                with transaction.atomic():
                    ...
                    checkouts_to_close = Checkout.objects.filter(
                        user=request.user)
                    checkouts_to_close = checkouts_to_close.exclude(
                        token=token)
...
def change_checkout_user(checkout, user):
    ...
    checkout.save(
        update_fields=['user', 'shipping_address', 'billing_address'])
...
def _process_shipping_data_for_order(checkout, taxes):
    ...
    if checkout.user:
        store_user_address(checkout.user, shipping_address,
...
def is_fully_paid(checkout: Checkout, taxes, discounts):
    ...
    checkout_total = checkout.get_total(
        discounts=discounts, taxes=taxes).gross.amount
