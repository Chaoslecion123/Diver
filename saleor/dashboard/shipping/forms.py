...
from ...world.models import CountryArea, City, CityArea
...
class ShippingZoneForm(forms.ModelForm):
    class Meta:
        ...
        help_texts = {
            ...
            'national': pgettext_lazy(
                'Help text for ShippingZone name',
                'If selected, this zone will used as national shipping zone.'
                'Please, select only one country to use this option.')}
...
class LocationShippingMethodForm(forms.ModelForm):
    country_area = forms.ModelChoiceField(
        queryset=CountryArea.objects.order_by('name'),
        required=True,
        label=pgettext_lazy(
            'Location shipping method form field label',
            'Departament'))
    city = forms.ModelChoiceField(
        queryset=City.objects.order_by('country_area__name', 'name'),
        required=False,
        label=pgettext_lazy(
            'Location shipping method form field label',
            'Province'))
    city_area = forms.ModelChoiceField(
        queryset=CityArea.objects.order_by('city__name', 'name'),
        required=False,
        label=pgettext_lazy(
            'Location shipping method form field label',
            'District'))
    ...
    class Meta(ShippingMethodForm.Meta):
        fields = [
            'name', 'price', 'country_area', 'city', 'city_area']
        labels = ShippingMethodForm.Meta.labels
# END :: SoftButterfly Extensions ----------------------------------------------


def get_shipping_form(shipping_type):
    if shipping_type == ShippingMethodType.WEIGHT_BASED:
        return WeightShippingMethodForm
    elif shipping_type == ShippingMethodType.PRICE_BASED:
        return PriceShippingMethodForm
    elif shipping_type == ShippingMethodType.LOCATION_BASED:
        return LocationShippingMethodForm
    raise TypeError('Unknown form type: %s' % shipping_type)
