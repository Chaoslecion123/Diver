...
from .forms import (
    AuthorizationKeyForm, SiteForm, SiteSettingsForm, PaymentSettingsForm,
    DashboardStyleSettingsForm, StoreStyleSettingsForm)
...
@staff_member_required
@permission_required('site.manage_settings')
def payment_settings_edit(request, pk):
    site_settings = get_object_or_404(
        SiteSettings,
        pk=pk, )
    ...
    return TemplateResponse(
        request,
        'dashboard/sites/payment/form.html',
        ctx, )


# Dashboard style settings view ------------------------------------------------
@staff_member_required
@permission_required('site.manage_settings')
def dashboard_style_settings_edit(request, pk):
    site_settings = get_object_or_404(
        SiteSettings,
        pk=pk, )

    dashboard_settings_form = DashboardStyleSettingsForm(
        request.POST or None,
        request.FILES or None,
        instance=site_settings, )

    if dashboard_settings_form.is_valid():
        dashboard_settings = dashboard_settings_form.save()
    ...
    ctx = {
        'site_settings': site_settings,
        'dashboard_settings': dashboard_settings_form, }

    return TemplateResponse(
        request,
        'dashboard/sites/dashboard_styles/form.html',
        ctx, )


# Storefront style settings view -----------------------------------------------
@staff_member_required
@permission_required('site.manage_settings')
def store_style_settings_edit(request, pk):
    site_settings = get_object_or_404(
        SiteSettings,
        pk=pk, )

    store_settings_form = StoreStyleSettingsForm(
        request.POST or None,
        request.FILES or None,
        instance=site_settings, )

    if store_settings_form.is_valid():
        store_settings = store_settings_form.save()
        store_settings.save()
    ...
    ctx = {
        'site_settings': site_settings,
        'store_settings': store_settings_form, }

    return TemplateResponse(
        request,
        'dashboard/sites/store_styles/form.html',
        ctx, )


# Storefront footer menu views -------------------------------------------------
