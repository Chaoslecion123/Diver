...
class ProductForm(forms.ModelForm, AttributesMixin):
    ...
    brand = forms.ModelChoiceField(
        ...
        label=pgettext_lazy('Brand select', 'Brand'))

    specs = RichTextField(
        label=pgettext_lazy('Specs', 'Especificaciones'), required=False)
    # END :: SoftButterfly Extensions ------------------------------------------
    ...
    def __init__(self, *args, **kwargs):
        ...
        # BEGIN :: SoftButterfly Extensions ------------------------------------
        if isinstance(self.instance.brand, Brand):
            self.fields['brand'].initial = self.instance.brand.pk
        # END :: SoftButterfly Extensions --------------------------------------
