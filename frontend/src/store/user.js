...
const VERIFY_BEGIN = 'VERIFY_BEGIN'
const VERIFY_SUCCESS = 'VERIFY_SUCCESS'
const VERIFY_FAILURE = 'VERIFY_FAILURE'
const VERIFY_SET_MESSAGES = 'VERIFY_SET_MESSAGES'
const VERIFY_SET_STATUS_CODE = 'VERIFY_SET_STATUS_CODE'
...
const actions = {
  create ({ commit }, { data, config = {} } = {}) {
    ...
    return USER.create(data, config)
      .then(response => {
  ...
  read ({ commit }, { id, query = {}, config = {} } = {}) {
    ...
    return USER.retrieve(id, query, config)
      .then(response => {
        commit(SET_OBJECT, response.data)
      })
      .then(() => {
  ...
  update ({ commit }, { id, data, config = {} } = {}) {
    ...
    return USER.update(id, data, config)
      .then(response => {
        commit(SET_OBJECT, response.data)
      })
      .then(() => {
  ...
  verify ({ commit }, { data, config = {} } = {}) {
    commit(VERIFY_BEGIN)
    commit(VERIFY_SET_MESSAGES, null)
    commit(VERIFY_SET_STATUS_CODE, null)
    return USER.verify(data, config)
      .then(response => {
        commit(SET_VERIFY, response.data)
      })
      .then(() => {
      ...
      .catch(error => {
        ...
        if (error.response.status === 404) {
          commit(SET_VERIFY, error.response.data)
        } else {
          commit(VERIFY_SET_MESSAGES, error.response.data)
          commit(VERIFY_SET_STATUS_CODE, error.response.status)
        }
      })
  }
}
...
const mutations = {
  ...
  [VERIFY_SET_MESSAGES] (state, messages) {
    state.verifyMessages = messages
  },
  [VERIFY_SET_STATUS_CODE] (state, statusCode) {
    state.verifyStatusCode = statusCode
  },
  /**
   * Handle objects
   */
