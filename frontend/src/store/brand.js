...
const SET_FEATURED_OBJECTS = 'SET_FEATURED_OBJECTS'
const initialState = {
  schema: null,
  object: null,
  objects: [],
...
const actions = {
  ...
  listFeatured (
    ...
    return BRAND
      ...
      .then((response) => {
        commit(SET_FEATURED_OBJECTS, response.data)
      })
      .then(() => {
        commit(LIST_SUCCESS)
      })
...
const mutations = {
  ...
  [SET_FEATURED_OBJECTS] (state, objects) {
    state.featuredObjects = objects
  }
}
